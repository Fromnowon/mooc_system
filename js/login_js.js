...
function uiSet() {
    ...
    var login_form = $("#login");
    login_form.css({height: $('#login_form').height() + 50});
    var footer = $("footer");
    footer.css('height', 30);
}
...
function formCheck() {
    ...
    function tipHandler(object1, object2, min) {
        ...
        if (object1.val().length > 0 && object1.val().length < min) {
            ...
            object1.parent().next().css('visibility', 'visible');
        }
        else {
            object1.parent().next().css('visibility', 'hidden');
...
//异步登录
function ajaxLogin() {
    if (typeof($("#submit_btn").attr("disabled")) != "undefined") {
        alert('请检查登录信息');
        return false;
    }
    $('.login_loader').fadeIn();
